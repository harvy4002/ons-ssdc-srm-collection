...
import uk.gov.ons.ssdc.rhservice.client.ExceptionManagerClient;
import uk.gov.ons.ssdc.rhservice.model.dto.ExceptionReportResponse;
import uk.gov.ons.ssdc.rhservice.model.dto.SkippedMessage;
...
class ManagedMessageRecovererTest {
  ...
  void testRecoverLogIt() {
    ...
    verify(exceptionManagerClient)
        .reportException(
            ...
            contains(
                "uk.gov.ons.ssdc.rhservice.messaging.ManagedMessageRecovererTest.testSetupTestRecover"));
    verify(originalMessage, never()).nack();
    verify(originalMessage, never()).ack();
    assertThat(thrownException.getMessage())
        .isEqualTo("Cannot process this message at this time, but it will be retried");
  ...
  void testRecoverSkip() {
    ...
    assertThat(skippedMessage.getService()).isEqualTo("RH Service");
  }
